            ...
            def analyze_ticker(ticker, settings, token, chat_id):
                ...
                try:
                    ...
                    if settings.get('📰 뉴스') or settings.get('🏛️ SEC'):
                        ...
                        # 빈도 조절: 한 번에 최대 2개까지만 전송
                        count = 0
                        for item in items:
                            if count >= 2: break 
                            
                            # [핵심] 고유 ID 생성 (제목 + 원본 시간)
                            # hashlib을 사용해 긴 문자열을 짧은 고유 코드로 변환
