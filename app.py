...
def write_log(msg):
    ...
    # print(f"[{timestamp}] {msg}") # 로그 과다 방지
    try:
    ...
    def run_bot_system():
        ...
        try:
            ...
            def sec_cmd(m):
                try:
                    ...
                    if items:
                        msg = [f"🏛️ *{t} SEC*"]
                        for i in items[:5]: 
                            msg.append(f"▪️ `[{i['date']}]` [{i['title'].replace('🏛️ ','').replace('[','').replace(']','')}]({i['link']})")
                        bot.reply_to(m, "\n\n".join(msg), parse_mode='Markdown', disable_web_page_preview=True)
            ...
            def analyze_ticker(ticker, settings, token, chat_id):
                ...
                try:
                    # 1. 뉴스 및 공시 (Race Condition Fix: 저장 직전에 로드)
                    if settings.get('📰 뉴스') or settings.get('🏛️ SEC'):
                        # 설정 로드는 루프 바깥의 것을 참조하되, 히스토리 저장은 신중하게
                        ...
                        if updated:
                            # [핵심 수정] UI 설정(settings)이 덮어씌워지지 않도록
                            # 저장 직전에 '가장 최신 파일'을 다시 읽어서 news_history만 교체
                            fresh_config = load_config()
                            fresh_config['news_history'] = history 
...
with t2:
    ...
    # [UI 수정 부분: 항목별 일괄 체크박스]
    if config['tickers']:
        # 첫 번째 티커에서 키 목록 추출
        first_keys = list(next(iter(config['tickers'].values())).keys())
        
        # Streamlit 컬럼 생성 (첫 번째 컬럼은 종목명 자리이므로 비워둠)
        # 비율: 종목명(1.5) : 옵션들(1:1:1...)
        cols = st.columns([1.5] + [1] * len(first_keys))
        
        # 첫 번째 컬럼(종목명 위)은 비워둠
        cols[0].write("")
        
        has_changed_master = False
        
        # 각 옵션 키에 대해 "Master Checkbox" 생성
        for idx, key in enumerate(first_keys):
            # 현재 모든 티커에 대해 해당 옵션이 True인지 확인
            is_all_true = all(config['tickers'][t].get(key, False) for t in config['tickers'])
            
            # 헤더 위치에 체크박스 배치
            new_val = cols[idx+1].checkbox(key, value=is_all_true, key=f"master_{key}")
            
            # 체크박스 값이 현재 상태와 다르면 -> 사용자가 클릭함
            if new_val != is_all_true:
                for t in config['tickers']:
                    config['tickers'][t][key] = new_val
                has_changed_master = True

        if has_changed_master:
            save_config(config)
            st.rerun()

        # 기존 데이터 에디터 렌더링
        df = pd.DataFrame(config['tickers']).T
        # 컬럼 순서를 마스터 체크박스 순서와 일치시킴
        df = df[first_keys]
        
        edited = st.data_editor(df, use_container_width=True, height=len(df)*35 + 38)
        
        if not df.equals(edited):
            # [중요] 봇과 충돌 방지를 위해, 여기서도 최신 Config를 로드한 뒤 Tickers만 병합
            latest_conf = load_config()
            latest_conf['tickers'] = edited.to_dict(orient='index')
    ...
    input_t = c_add_1.text_input("Add Tickers", label_visibility='collapsed', placeholder="티커 입력 (예: AAPL, TSLA)")
    if c_add_2.button("➕ Add", use_container_width=True):
        if input_t:
    ...
    del_target = c_del_1.selectbox("Delete Ticker", options=list(config['tickers'].keys()), label_visibility='collapsed')
    if c_del_2.button("🗑️ Del", use_container_width=True):
